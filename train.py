...
def train():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (img_batch, category_batch, attribute_batch, visibility_batch, landmark_batch) in enumerate(train_dataloader):
            ...
            # new
            if epoch < cov_epoch:
                criterion = criterions['cov']
                loss, cat_loss, att_loss, vis_loss, lm_loss, loss_weights = criterion(
                ...
                creterion = criterions['base']
                cat_loss, att_loss, vis_loss, lm_loss = creterion(
                    preds = (category_out, attr_out, vis_out, loc_out),
                    targets = (category_batch, attribute_batch, visibility_batch, landmark_batch),
                )
        ...
        with torch.no_grad():
            ...
            for img_batch, category_batch, attribute_batch, visibility_batch, landmark_batch in valid_dataloader:
                ...
                if epoch < cov_epoch:
                    criterion = criterions['cov']
                    val_loss, cat_val_loss, att_val_loss, vis_val_loss, lm_val_loss = criterion(
                        preds = (category_out, attr_out, vis_out, loc_out),
                        targets = (category_batch, attribute_batch, visibility_batch, landmark_batch),
                        mode='valid'
                    )
                else:
                    criterion = criterions['base']
                    cat_val_loss, att_val_loss, vis_val_loss, lm_val_loss = criterion(
                        preds = (category_out, attr_out, vis_out, loc_out),
                        targets = (category_batch, attribute_batch, visibility_batch, landmark_batch),
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cov_epoch", type=int, default=15)
    parser.add_argument("--att_weight", type=float, default=3.0)
